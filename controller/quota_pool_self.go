...
func GetSelfQuotaPoolTransactions(c *gin.Context) {
	...
	page := common.GetPageQuery(c)
	items, total, err := model.ListQuotaPoolTransactions(pool.Id, page, c.Query("type"), c.Query("keyword"))
	if err != nil {
		writeQuotaPoolError(c, err)
		return
	}
...
func GetSelfQuotaPoolOperationLogs(c *gin.Context) {
	...
	page := common.GetPageQuery(c)
	items, total, err := model.ListQuotaPoolOperationLogs(pool.Id, page, c.Query("action"))
	if err != nil {
		writeQuotaPoolError(c, err)
		return
	}
...
func AddSelfQuotaPoolMember(c *gin.Context) {
	...
	result, err := model.AddUserToQuotaPool(req.UserId, pool.Id, c.GetInt("id"))
	if err != nil {
		writeQuotaPoolError(c, err)
		return
	}
...
func MoveSelfQuotaPoolMember(c *gin.Context) {
	...
	if !requireSelfQuotaPoolMemberRemoval(c, admin) {
		return
	}
	...
	target, err := model.GetNewUserQuotaPool()
	if err != nil {
		writeQuotaPoolError(c, err)
		return
	}
	if req.PoolId != target.Id {
		writeQuotaPoolError(c, model.ErrQuotaPoolPermissionDenied)
		return
	}
	removeQuotaPoolMember(c, pool.Id, userId, false)
}

func RemoveSelfQuotaPoolMember(c *gin.Context) {
	pool, admin, ok := selfQuotaPool(c)
	if !ok {
		return
	}
