...
- `fields`：更新配置时实际发生变化的字段数量。
- `changes`：配置更新的变更列表，每项包含 `field`、`before` 和 `after`。在更新配置的事务内持有池行锁后生成快照，只包含实际变化的业务配置字段，不包含派生的 `quota` 更新表达式。金额保持内部额度单位，名称使用规范化后的值；事务失败时不记录成功审计。
- `enabled`：启用或停用后的状态。
...
临时充值按整个额度池累计，每自然月最多 3 次（按服务器时区划分），不区分操作人；单次金额仍不得超过当时基础额度的 50%。被拒绝的充值不改变余额，也不写入成功充值流水。接口保留 HTTP 409 和 `QUOTA_POOL_REFILL_LIMITED`，错误提示分别说明基础额度无效、单次金额超过基础额度的 50%、本月已达 3 次上限。

名称缺失时使用 `#ID`，金额通过现有 `formatQuota` 渲染。布尔状态和字段数量在前端转换为本地化文本。

## 前端展示

资金流水与操作日志均使用服务端分页，默认每页 10 条，可选 20、50、100 条。两类列表独立保存页码和每页条数；切换条数回到第 1 页，查询缓存包含池 ID、访问范围、页码和条数。管理端与池管理员端沿用现有 `p`、`page_size` 参数及 `total` 返回值，加载期间禁用分页控件。

资金流水支持 `type` 精确筛选及 `keyword` 搜索。关键词对交易的用户和操作人匹配用户名、昵称（不区分大小写、按普通文本包含匹配），纯数字同时匹配用户 ID；搜索不要求用户仍属于当前池。流水展示用户与操作人，查不到姓名时回退 ID。操作日志支持 `action` 精确筛选，对应 `other.op.action`；没有结构化 action 的历史日志仍可在“全部”中查看。两类过滤均在计数和分页前执行，保留当前池与角色权限边界；资金流水使用主库，操作日志使用日志库，不增加跨库关联。切换类型、提交关键词或切换操作时重置为第 1 页。
...
配置更新日志在原摘要下逐行展示“字段：旧值 → 新值”，同时覆盖管理端 `quota_pool.update` 和池管理员端 `quota_pool.self_update`。充值金额 `-1` 显示继承系统设置、`0` 显示禁用；周/月次数 `-1` 显示继承、`0` 显示不限。旧日志缺少 `changes` 时保留原摘要，不根据当前配置推测历史值。
