...
const loginMock = vi.hoisted(() => ({ dingtalk: vi.fn() }))
...
vi.mock('../../hooks/use-oauth-login', () => ({
  useOAuthLogin: () => ({
    ...
    handleDingTalkLogin: loginMock.dingtalk,
...
test('shows a DingTalk scan-login button when enabled', async () => {
  const user = userEvent.setup()
  render(
    <OAuthProviders
      status={{ dingtalk_login: true, dingtalk_client_id: 'app-key' }}
    />
  )

  const button = screen.getByRole('button', { name: 'Continue with DingTalk' })
  expect(button).toBeInTheDocument()
  await user.click(button)
  expect(loginMock.dingtalk).toHaveBeenCalledOnce()
})
